...
- **Publication entries** follow a fixed markup contract that `index.css` styles by class: `.publication-card` rows (renamed from `.publication-block` in the 2026-06-23 StyleSeed restyle — see `STYLESEED.md`), venue pills `.venue-badge .venue-{cvpr,iclr,neurips,iccvw,arxiv,…}` (classes kept for semantics but all render as one neutral grayscale pill), award pills `.award-badge`, and `.author-me` to bold Jiwon's name. The CV page uses the lighter `.publication-compact` markup. Reuse these classes rather than inventing new ones.
- **Design tokens (`css/index.css` `:root`)** and the card system (`.card-hero`, `.card-section`, `.publication-card` — StyleSeed's Four Section Types) were introduced 2026-06-23; see `STYLESEED.md` for the full design lock (accent color, grayscale scale, radius/shadow language, motion seed). New UI should reuse these tokens/classes rather than hardcoding colors or introducing a second radius/shadow language.
